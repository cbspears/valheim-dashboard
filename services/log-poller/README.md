...
Tails the Valheim dedicated-server log over **FTP** (`BepInEx/LogOutput.log` on G-Portal) and
...
## How it works
- Fetches only the **new bytes** of the log each poll (FTP resume offset), so 20s polling is cheap.
...
npm install
cp .env.example .env   # then fill in FTP creds + WEBHOOK_SECRET
```

## Test (no server activity needed)
```bash
npm run test:parser   # offline: validates parsing against fixtures/sample-session.log
npm run test:ftp      # connects to the real FTP, shows log tail + matched patterns
```

## Run
```bash
npm start
# or against a local log file instead of FTP:
...
| Var | Default | Notes |
|-----|---------|-------|
| `LOG_SOURCE` | `ftp` | `ftp` or `file` |
| `LOG_PATH` | `BepInEx/LogOutput.log` | remote (ftp) or local (file) path |
| `FTP_HOST/PORT/USER/PASSWORD` | — | G-Portal FTP (plain, not SFTP) |
