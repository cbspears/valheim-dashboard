...
#
# Twin of ../eilif-companion/refresh-libs.sh (same toolchain, same BepInEx pack).
# EilifBoards reads/writes sign ZDOs (ZDO / ZDOMan / ZDOVars / ZNet) and does HTTP —
# no Splatform, no PhysicsModule, no Harmony patches (0Harmony is referenced only because
# BepInEx.dll's public surface mentions it; the plugin itself patches nothing).
set -euo pipefail
...
# --- BepInEx reference DLLs (BepInExPack_Valheim 5.4.2333, matches the Eilif server) ---
