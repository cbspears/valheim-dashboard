...
## ValheimPlus on Valheim 1.0.12 (1.7.2)

Valheim's 1.0.10/1.0.12 hotfix changed `PlayerProfile.s_bypassCheatChecks` from a field into a property. ValheimPlus 10.0.2 and 10.0.3 were compiled against the field, and two of their patches still read it that way, so on 1.0.12 they throw the moment the game reaches them: smelters, kilns, furnaces, windmills, spinning wheels and fermenter taps simply stop producing while ValheimPlus is loaded. The `[VPlusHotfixShim]` section, on by default, removes those two patches and only those two — the prefix on `Smelter.Spawn` and the transpiler on `Fermenter.DelayedTap`. Everything else ValheimPlus does is untouched, and the one feature that goes with them is auto-deposit into a nearby chest, which this server has switched off. ValheimPlus re-applies its patches every time you join a server, so the shim re-strips them on every re-apply and re-checks every 30 seconds besides. If ValheimPlus is not installed the section is inert and says so. Turn it off with `[VPlusHotfixShim] Enabled = false` once ValheimPlus ships a build for 1.0.12 or later.
...
```
[EilifPaths] Eilif Paths v1.7.2 loaded. ... Core patch classes: 8/8 applied.
