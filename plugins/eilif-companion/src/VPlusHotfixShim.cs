...
{
    /// <summary>
    /// ValheimPlus hotfix shim (0.3.4). Identical in intent and in wording to the copy in the
    /// client plugin (../eilif-paths/src/VPlusHotfixShim.cs) — the dedicated server loads the same
    /// ValheimPlus DLL and hits the same MissingFieldException, so both sides need it. Removes exactly TWO ValheimPlus Harmony patches and
    ...
    /// hand (same convention as PlayFabPlayerCap.Apply), so `patch classes applied: 2/2` means
    /// exactly what it meant in 0.3.3.
    ///
    /// SERVER SIDE, TOO. On a dedicated server ValheimPlus still calls ReapplyPatches on
    /// "Config source changed", so the PatchAll postfix and the periodic re-check both earn their
    /// keep here as well.
    ///
    /// NOTE: no System.ValueTuple anywhere in this file — see BUILD.md.
    /// </summary>
    internal static class VPlusHotfix
    {
        ...
        internal static void Apply(Harmony harmony)
        {
            ...
            try
            {
                ...
                {
                    EilifCompanionPlugin.Log.LogInfo("[Eilif] VPlusHotfixShim: disabled in config " +
                ...
                if (vplus == null)
                {
                    EilifCompanionPlugin.Log.LogInfo("[Eilif] VPlusHotfixShim: inert - ValheimPlus is not " +
                ...
                EilifCompanionPlugin.Log.LogInfo("[Eilif] VPlusHotfixShim: Smelter.Spawn prefix removed (" +
                ...
                EilifCompanionPlugin.Log.LogInfo("[Eilif] VPlusHotfixShim: after - " +
                ...
                {
                    EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: ValheimPlus is loaded but " +
            ...
            {
                // Never take the plugin down over the shim: everything else in Eilif Companion is
                // independent of ValheimPlus.
                EilifCompanionPlugin.Log.LogError("[Eilif] VPlusHotfixShim: failed to apply: " + ex.Message +
        ...
        private static void ReSweep(string reason)
        {
            ...
            try
            {
                ...
                if (smelter > 0 || fermenter > 0)
                {
                    EilifCompanionPlugin.Log.LogInfo("[Eilif] VPlusHotfixShim: " + reason +
            ...
            {
                EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: re-check failed (" + reason +
                    "): " + ex.Message);
            }
                                 ...
                                 string patchClassFragment)
        {
            if (original == null)
            {
                EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: " + targetLabel +
            ...
            {
                EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: could not read patch info for " +
            ...
            foreach (Patch p in candidates)
            {
                ...
                {
                    EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: " + targetLabel + " carries a " +
                ...
                {
                    EilifCompanionPlugin.Log.LogError("[Eilif] VPlusHotfixShim: could not unpatch " + declName +
        ...
        private static void HookRepatch(Type vplusPluginType)
        {
            ...
            {
                EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: ValheimPlus is loaded but its " +
            ...
            if (patchAll == null)
            {
                EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: " + VPlusPluginTypeName +
            ...
            {
                EilifCompanionPlugin.Log.LogWarning("[Eilif] VPlusHotfixShim: could not hook " +
