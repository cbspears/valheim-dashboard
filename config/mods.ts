...
export const ALL_MODS: Mod[] = [
  ...
  },
  {
    ...
    version: '10.1.0', // 2026-09-11 14:02: Grantapher's 1.0.12 build; on the box since the afternoon window. r2modman installs the newest V+ on every profile update and V+ refuses a client newer than the server, so the box follows every V+ release the same hour. Same cfg schema as 10.0.2.
    category: 'QoL',
    clientRequired: true,
