...
export const ALL_MODS: Mod[] = [
  ...
  },
  {
    ...
    version: '10.1.2', // 2026-09-14 04:00 CT: auto-deposit-on-arrival fix, AutoStack message fix, inventory-rows mod compat, cultivator GridAlignment, procreation fixes; pack v20 pins it, box window staged (run-vplus1012-window.sh). r2modman installs the newest V+ on every profile update and V+ refuses a client newer than the server, so the box follows every V+ release the same hour. Same cfg schema as 10.0.2 (only a description string changed).
    category: 'QoL',
    clientRequired: true,
