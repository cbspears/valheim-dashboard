...
export function renderPack({
  ...
  for (const mod of MODS) {
    ...
    if (mod.cfgVersionVar) {
      ...
      // template body, so it must not drift just because the pin moved.
      if (!mod.cfgVersionDefault) {
        throw new Error(`renderPack: ${mod.label} has cfgVersionVar but no cfgVersionDefault`);
      }
      const cfgV = cfgVersions[mod.key] ?? mod.cfgVersionDefault;
  ...
  const files = new Map();
  for (const f of PACK_FILES) files.set(f.out, fill(readTemplate(f.tmpl), vars, f.tmpl));
  for (const cfg of CFG_FILES) {
    files.set(`config/${cfg}`, fill(readTemplate(path.join('config', `${cfg}.tmpl`)), vars, cfg));
  }
  return { files, vars };
}

/** The Mac bundle's README, rendered off the same template set. */
export function renderReadme({ packNumber, packDate }) {
  const title = `Eilif config bundle - Pack v${packNumber} (${packDate})`;
  return fill(readTemplate('README.txt.tmpl'), {
    BUNDLE_TITLE: title,
...
async function main(argv) {
  ...
  const pins = MODS.map((mod) => ({ mod, version: args.versions[mod.key] ?? mod.baseline }));
  ...
  if (args.publish && roundTripFailed) {
    ...
    console.log('    3. config/mods.ts: bump the version shown for every mod that moved');
