...
**Current state:** dashboard live (Vercel); Discord bot live (systemd `eilif-discord-bot`, recaps gated until launch); log poller built but not running (awaits server launch); stats pipeline = next build.
...
**Next task:** build the `.fch` stats parser (ServerCharacters → `player_stats`) — see the tracker.
