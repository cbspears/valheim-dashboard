...
- **Supabase** — `players, sessions, events, player_stats, bosses, roadmap, server_status, discord_events`. Public-read RLS; writes go through `/api/webhook` (service-role + `x-webhook-secret`). `occurredAt` + `sync` + `events_sync` supported.
