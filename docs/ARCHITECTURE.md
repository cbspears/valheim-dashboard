...
| `titles` | `TITLES_INTERVAL_MS`, default 10 m | reads `/api/titles`, announces changes, writes `title_history`. Sticky since 2026-09-10: an earned title is held against a hall-name offer, silent placeholder reshuffles, two-pass confirmation (`TITLE_CONFIRM_MS`), 24 h tenure (`TITLE_MIN_TENURE_MS`) on a viking's own move between earned titles, max `TITLES_PER_DAY` (2) a rolling day, and since 2026-09-17 no two proclamations within `TITLES_MIN_GAP_MS` (4 h) of each other (rule 8), which also holds a pass to a single proclamation. **One holder per earned title since 2026-09-16**: a confirmed offer of a title somebody wears is a handover, not a duplicate, and it spends one proclamation (one line per viking moved) rather than two. Since 2026-09-17 the handover target must itself be free (falling back to the `/api/titles` `placeholder`), every wearer of the title is handed off, and a takeover waits out `TITLE_TAKEOVER_COOLDOWN_MS` (24 h) since the title last changed hands. A pre-existing duplicate the engine has already picked a side on is closed by the stale wearer **yielding** (rule 1c), one quiet line. See *Living Titles* below |
...
Rule **8** (2026-09-17) makes proclamations rare as well as safe: none goes out within
`TITLES_MIN_GAP_MS` (4 h) of the last `title_history` row, and because the first proclamation of a
pass sets that clock to now, every other candidate in the pass is deferred to a later one — so a
budget that frees several slots at once can no longer empty as a batch, as it did at 12:51 that day.
Deferred candidates stay pending; `TITLES_MIN_GAP_MS=0` restores the old behaviour.

A duplicate that PREDATES all of this has no challenger to trigger a handover (the engine's wearer
reads *unchanged*), so rule **1c** closes it from the other side: the wearer the engine does not name
**yields** after the usual confirmation — one quiet line, `**Thorfinn** yields **Bane of Beasts** to
**Asbjorn** and takes up **the Cheerful Ballast**.` — for one budget slot, exempt from tenure and
from the cool-down.
...
| `/players` | Vikings: roster, the oath wall (`#oaths`), leaderboards, attendance grid, How We Die, anglers |
...
| `/mods`, `/commands`, `/oath` | gone as pages since 2026-09-06; all three are 308s in `next.config.ts`, to `/resources#mods`, `/resources#commands` and `/players#oaths`. A link inside the site must address the destination directly: a client-side `<Link>` to a redirected route loses the `#anchor` |
...
| `/api/voice` | GET | `x-voice-token` vs `VOICE_API_TOKEN` | service role | `voice_lines` has no public-read policy on purpose. Claims up to 3 queued lines and flips them to spoken. A line whose `meta.target` holds a non-empty character name (trimmed, at most 64 characters) is handed over with a `target` field and spoken in-game to that one peer, matched case-insensitively against `ZNetPeer.m_playerName`; anything else — absent, blank, over-long, not a string, `meta` null — omits the field and is spoken to everybody exactly as before. **Targeting fails closed in both halves.** In-game, a targeted line whose viking is not connected is **dropped** rather than broadcast. Here, a targeted line is handed over **only** to a caller whose `x-eilif-caps` header advertised `targeting` — a Companion older than 0.3.3 ignores the unknown member and would speak a private line to the hall, so it is not given the line at all (the row is still marked spoken, since three stuck targeted rows at the head of the queue would silence the hall entirely; the withholding is logged to the Vercel runtime log). The same self-report is also written to the `companion-voice` heartbeat as `metrics.targeting`, but that is **cockpit display only** and never a control input: `recordRouteHeartbeat` is throttled to one write a minute per instance, so the stored value belongs to whichever caller won the slot. |
...
| `plugins/eilif-paths` | client, ships in the pack | **1.6.0** built and staged 2026-09-06 (`plugins/thunderstore/EilifPaths-1.6.0/`, the version the v12 mint pins); **1.5.0** is what is published on Thunderstore (2026-09-06 10:01 CT) and what pack v11 players run; `dist` rebuilt 2026-09-06, still against the 0.221.12 assemblies | `net.eilif.paths` | Always on: `Player.GetJogSpeedFactor`, `GetRunSpeedFactor`, `UseStamina`, `Character.UpdateWalking`, `Bed.CheckFire`, `StationExtension.Awake`, and since 1.6.0 `Minimap.UpdateExplore` (map discovery radius, x1.5 on foot / x2 on a ship, prefix + finalizer) and `Player.UpdateStats(float)` (stamina regen while swimming and treading, postfix) — the eight the `Core patch classes: 8/8` line counts, up from 6/6 in 1.5.0. Only when the V+ fallback is switched on: `CraftingStation.Start` and `CheckUsable` (the 30 m station build range and no-roof crafting, `VPlusFallbackPatch.cs`), `Fireplace.Awake`, `CookingStation.UpdateCooking`, `Smelter.UpdateSmelter`, `ShieldGenerator.*`, `Pickable.RPC_Pick`, `DropTable.GetDropList` and `CharacterDrop.GenerateDropList`. `Minimap.Explore` is called by reflection, not patched |
