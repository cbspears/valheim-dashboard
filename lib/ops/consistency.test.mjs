...
// ── Future-dated events → critical ────────────────────────────────────────
// The row shape that froze the #server relay's cursor for good (red-team,
// 2026-09-05). Nothing else in the cockpit surfaces it: a relay tick that posts
// nothing is a success, so every health signal stays green while the feed is
// dead.
{
  const i = clean();
  i.futureDatedEvents = [
    { type: 'death', character_name: 'TrollX', created_at: '2999-01-01T00:00:00Z' },
  ];
  const f = runConsistencyChecks(i);
  const found = f.find((x) => x.id === 'future-dated-events');
  ok(found !== undefined, 'a future-dated event row is reported');
  ok(found.severity === 'critical', 'and it is critical, not a warning');
  ok(found.detail.includes('2999-01-01T00:00:00Z'), 'the offending timestamp is in the evidence');
  ok(found.detail.includes('TrollX'), 'and so is the name on the row');
}
{
  // An empty list is the normal case and must stay silent.
  ok(!ids(runConsistencyChecks(clean())).has('future-dated-events'), 'no rows → no finding');
}
