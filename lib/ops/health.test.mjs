...
import { computeState, buildHealth, COMPONENTS, SERVER_STATUS_TRUST_SEC } from './health.ts';
...
// lib/ops/db hands it over without the cockpit page needing to know).
{
  ...
  // Passed explicitly. The cockpit page now hands this over by name as well as
  // letting it ride on the heartbeat row, so the signal does not depend on the
  // row's metrics having been merged.
  ...
  // Precedence: the named input wins over whatever the row happens to carry, so
  // the page always shows the number it actually read this render.
  const both = buildHealth({
    nowMs: NOW, supabaseOk: true, dashboardVersion: null, serverStatusUpdatedAt: ago(30),
    heartbeats: { 'companion-voice': fresh({ voiceQueueOldestSec: 15 * 60 }) },
    voiceQueueOldestSec: 30,
  });
  ok(find(both, 'companion-voice').state === 'healthy',
    'a fresh explicit reading overrides a stale one carried on the row');
}

// ── an empty hall is not a dead voice half ────────────────────────────────
// The Companion polls /api/voice only while >=1 player is connected, so five
// minutes after the last viking logs off its heartbeat ages out. Without the
// roster count that turned the cockpit red every night for correct behaviour.
{
  const voiceHb = (agoSec) => ({
    'companion-voice': {
      component: 'companion-voice', instance: null, version: null, status: 'ok',
      last_success: ago(agoSec), last_attempt: ago(agoSec), error_summary: null,
      metrics: {}, updated_at: ago(agoSec),
    },
  });
  const base = { nowMs: NOW, supabaseOk: true, dashboardVersion: null, serverStatusUpdatedAt: ago(30) };

  const quiet = buildHealth({ ...base, heartbeats: voiceHb(3000), playersOnline: 0 });
  ok(find(quiet, 'companion-voice').state === 'unknown',
    'nobody online + an aged voice heartbeat -> unknown, not a nightly false red');
  ok(/only polls while a viking is connected/.test(find(quiet, 'companion-voice').detail),
    'and the detail says plainly why it is quiet');

  const busy = buildHealth({ ...base, heartbeats: voiceHb(3000), playersOnline: 3 });
  ok(find(busy, 'companion-voice').state === 'stale',
    'players online + an aged voice heartbeat -> STILL stale (this is the real fault)');

  const fresh = buildHealth({ ...base, heartbeats: voiceHb(30), playersOnline: 0 });
  ok(find(fresh, 'companion-voice').state === 'healthy',
    'a fresh poll is healthy whatever the roster says');

  const omitted = buildHealth({ ...base, heartbeats: voiceHb(3000) });
  ok(find(omitted, 'companion-voice').state === 'stale',
    'omitting the roster count leaves staleness exactly as it was');

  // The quiet-hall rule is scoped to the one component that stops polling.
  const poller = buildHealth({
    ...base,
    heartbeats: { 'log-poller': { component: 'log-poller', instance: null, version: null, status: 'ok', last_success: ago(3000), last_attempt: ago(3000), error_summary: null, metrics: {}, updated_at: ago(3000) } },
    playersOnline: 0,
  });
  ok(find(poller, 'log-poller').state === 'stale',
    'an empty hall never excuses the log poller, which tails the log regardless');

  // The roster comes OUT of server_status, so a dead emitter reports an empty
  // hall whether or not anyone is playing. Believing it there would downgrade a
  // genuinely dead voice half from stale to unknown at exactly the wrong moment,
  // and the one component that would contradict it (server-emitter) is the one
  // that just died.
  const staleStatus = { ...base, serverStatusUpdatedAt: ago(3000) };
  const blindQuiet = buildHealth({ ...staleStatus, heartbeats: voiceHb(3000), playersOnline: 0 });
  ok(find(blindQuiet, 'companion-voice').state === 'stale',
    'an empty roster read off a STALE server_status does not excuse the voice half');
  ok(find(blindQuiet, 'server-emitter').state === 'stale',
    'and the emitter that produced that roster is itself flagged stale');

  const noStatus = buildHealth({
    ...base, serverStatusUpdatedAt: null, heartbeats: voiceHb(3000), playersOnline: 0,
  });
  ok(find(noStatus, 'companion-voice').state === 'stale',
    'no server_status at all is not evidence of an empty hall either');

  ok(find(buildHealth({ ...base, serverStatusUpdatedAt: ago(299), heartbeats: voiceHb(3000), playersOnline: 0 }), 'companion-voice').state === 'unknown',
    'just inside the trust window the roster is still believed');
  ok(find(buildHealth({ ...base, serverStatusUpdatedAt: ago(301), heartbeats: voiceHb(3000), playersOnline: 0 }), 'companion-voice').state === 'stale',
    'one second past it the roster is not');

  ok(SERVER_STATUS_TRUST_SEC === COMPONENTS.find((c) => c.key === 'server-emitter').staleAfterSec,
    'the trust window is read off the emitter component, so the two cannot drift');
}
