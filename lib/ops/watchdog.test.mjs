...
/** All four producers fresh + a fresh online server. */
...
{
  ok(WATCHDOG_TARGETS.length === 5, 'four producers + the game server are watched');
  ...
  for (const key of ['discord-bot', 'log-poller', 'map-snapshot', 'stats-parser', 'game-server']) {
...
// ── Never reported is NOT an alert (the stats-parser case today) ──────────
{
  const e = evaluateWatchdog({
    ...
    heartbeats: {
      ...
      'log-poller': hb('log-poller'),
      'map-snapshot': hb('map-snapshot'),
      // stats-parser row absent entirely — it gains its heartbeat today.
    },
    serverStatusUpdatedAt: ago(60),
    serverIsOnline: true,
  });
  ok(e.ok === true, 'a component that never reported does not trip the watchdog');
  ok(find(e, 'stats-parser').state === 'unknown', 'missing row → unknown');
  ok(find(e, 'stats-parser').unhealthy === false, 'unknown never alerts');
  ok(e.neverReported.includes('stats-parser'), 'never-reported is surfaced in the summary');

  // A row that exists but has never had a success is the same case.
  const e2 = evaluateWatchdog({
    nowMs: NOW,
    heartbeats: { 'stats-parser': hb('stats-parser', { lastSuccess: null, status: 'error' }) },
    serverStatusUpdatedAt: ago(60),
    serverIsOnline: true,
  });
  ok(find(e2, 'stats-parser').state === 'unknown', 'row with no last_success → unknown');
  ok(find(e2, 'stats-parser').unhealthy === false, 'no success yet → still not an alert');
}
...
// ── Fresh but self-reported error → degraded, and it alerts ───────────────
{
  const e = allHealthy({
    heartbeats: {
      'discord-bot': hb('discord-bot', { status: 'error', error: 'gateway closed' }),
      'log-poller': hb('log-poller'),
      'map-snapshot': hb('map-snapshot'),
      'stats-parser': hb('stats-parser'),
    },
...
// ── Message formatting ────────────────────────────────────────────────────
{
  ...
  const allDown = evaluateWatchdog({
    ...
    heartbeats: Object.fromEntries(
      ['discord-bot', 'log-poller', 'map-snapshot', 'stats-parser'].map((k) => [
        k,
  ...
  ok(allDown.unhealthy.length === 5, 'a PC-off outage flags every producer plus the server');
