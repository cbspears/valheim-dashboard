...
// ── titleContests ───────────────────────────────────────────────────────────
{
  ...
  // An EARNED title the engine wants to swap for a placeholder is HELD, not
  // flipped: the announcer refuses that demotion outright (Charlie, 2026-09-10),
  // so the row must never read as a proclamation that is running late.
  const held = titleContests([
    { name: 'Rosir', incumbent: 'the Ever-Present', stable: 'of the Quiet Fjord', raw: 'of the Quiet Fjord' },
  ]);
  eq(held[0].kind, 'held', 'earned title vs an offered placeholder is a hold');
  const promo = titleContests([
    { name: 'Rosir', incumbent: 'of the Quiet Fjord', stable: 'the Ever-Present', raw: 'the Ever-Present' },
  ]);
  eq(promo[0].kind, 'flipping', 'a placeholder giving way to an earned title is still a flip');
