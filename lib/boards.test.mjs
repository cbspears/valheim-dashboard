...
{
  const STAT_KEYS = ['kills', 'deaths', 'builds', 'resources', 'explored', 'distance'];
  const roster = [
    viking('Astrid', { kills: 12, deaths: 3, builds: 400, resources: 9_000, distanceM: 84_200, exploredPct: 31.94, title: 'the Provider' }),
    // Ties Astrid on kills AND builds, so two of the six leaders are decided by name.
    viking('Bjorn Ironside', { kills: 12, deaths: 9, builds: 400, resources: 4_000, distanceM: 12_000, exploredPct: 8 }),
    viking('Ceol', { kills: 5, deaths: 1, builds: 12, resources: 1, distanceM: 5, exploredPct: 0.4 }),
  ];
  ...
  // Six, not eight: Living Titles has no winner and Great Deeds is a warband total.
  assert.deepStrictEqual(Object.keys(leaders), STAT_KEYS, 'six stat plaques, no titles and no deeds');
  passed++;
