...
eq(
  ...
  'in 3h',
  '4:30pm CT the same day is not yet "tonight" — the hour count from the old card formatter'
);
eq(
...
// ── ONE FORMATTER, EVERY SURFACE (T-3 audit, site-3) ────────────────────────
// The nav pill and the Hall's Coming Up card used to call different functions and
// answered the same question differently: on 2026-09-06 the same screen carried
// "in 3 days" (pill, calendar days) and "in 4 days" (card, rounded 24-hour
// blocks) for Deep North Launch Night. These walk launch week hour by hour and
// assert a single answer per instant.
console.log('\ngatheringCountdown across launch week (one answer per instant)');
const LAUNCH = '2026-09-09T22:30:00Z'; // Wed 2026-09-09, 5:30pm CT
for (const [nowIso, expected, why] of [
  ['2026-09-06T05:30:00Z', 'in 3 days', 'Sun 00:30 CT — three sleeps away, never four'],
  ['2026-09-06T18:00:00Z', 'in 3 days', 'Sun 13:00 CT — same day, same answer'],
  ['2026-09-07T18:00:00Z', 'in 2 days', 'Mon afternoon'],
  ['2026-09-08T18:00:00Z', 'tomorrow', 'Tue afternoon -> tomorrow'],
  ['2026-09-09T13:00:00Z', 'tonight', 'Wed 08:00 CT — launch morning'],
  ['2026-09-09T20:00:00Z', 'tonight', 'Wed 15:00 CT — the go/no-go'],
  ['2026-09-09T22:00:00Z', 'in 30 min', 'half an hour out'],
  ['2026-09-09T22:30:00Z', 'happening now', 'the moment itself'],
]) {
  eq(gatheringCountdown(LAUNCH, Date.parse(nowIso)), expected, why);
}

// The regression guard with teeth: every surface that renders a countdown must
// import the same helper. A second formatter is what caused the contradiction.
console.log('\nevery countdown surface calls the same helper');
const { readFileSync } = await import('node:fs');
const src = (rel) => readFileSync(new URL(`../${rel}`, import.meta.url), 'utf8');
for (const file of [
  'components/events/NextGatheringPill.tsx',
  'components/events/UpcomingEvents.tsx',
  'app/tv/page.tsx',
  'lib/next-gathering.ts',
]) {
  ok(src(file).includes('gatheringCountdown'), `${file} uses gatheringCountdown`);
}
const formatSrc = src('lib/format.ts');
eq(
  (formatSrc.match(/export function \w*[Cc]ountdown\(/g) ?? []).length,
  1,
  'lib/format.ts exports exactly ONE countdown function'
);
